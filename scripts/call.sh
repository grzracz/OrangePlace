...
# Find the abi.json file in the build directory
abi=$(find build -name "*.arc4.json")

if [ -z "$abi" ]
then
  echo "Cannot find the abi.json file in the build directory. Please make sure it exists."
  exit 1
fi
...
NETWORK=$network \
node scripts/call.mjs $app_id ../$abi $method $methodArgs
